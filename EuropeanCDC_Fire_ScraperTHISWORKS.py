#!/usr/bin/env python3
"""
Enhanced ECDC Outbreak Scraper
Advanced version with better date filtering, disease analysis, and reporting
Scrapes European Centre for Disease Prevention and Control (ecdc.europa.eu)
"""
...
class ProMEDScraper:
    """Enhanced ECDC scraper with better filtering and analysis"""
    ...
    def extract_date_from_content(self, content: str, title: str) -> Optional[datetime]:
        ...
        Looks for common date patterns in ECDC posts
        """
        date_patterns = [
    ...
    def scrape_outbreaks(self, months_back: int = 6, max_results_per_query: int = 100) -> Dict[str, Any]:
        """Main scraping function"""
        start_date, end_date = self.get_date_range(months_back)
        print(f"\n{'='*70}")
        print(f"SCRAPING ECDC OUTBREAKS")
        ...
        search_queries = [
            ...
            f"site:ecdc.europa.eu human disease outbreak {current_year}",
            f"site:ecdc.europa.eu infectious disease human {current_year}",
            f"site:ecdc.europa.eu epidemic confirmed cases {current_year}",
            
            # Animal disease queries
            f"site:ecdc.europa.eu animal disease outbreak {current_year}",
            f"site:ecdc.europa.eu livestock disease {current_year}",
            f"site:ecdc.europa.eu avian influenza {current_year}",
            f"site:ecdc.europa.eu veterinary alert {current_year}",
            
            # Zoonotic queries
            f"site:ecdc.europa.eu zoonotic disease {current_year}",
            f"site:ecdc.europa.eu zoonosis outbreak {current_year}",
            
            # General outbreak queries
            f"site:ecdc.europa.eu outbreak alert {current_year}",
            f"site:ecdc.europa.eu disease surveillance {current_year}",
            f"site:ecdc.europa.eu public health emergency {current_year}",
        ]
    ...
    def generate_report(self, categorized_data: Dict[str, Any], output_file: str = "ecdc_outbreak_report.json"):
...
def main():
    ...
    print("\n" + "="*70)
    print("ENHANCED ECDC OUTBREAK SCRAPER")
    print("="*70 + "\n")
    ...
    try:
        ...
        # Scrape outbreaks
        categorized_data = scraper.scrape_outbreaks(months_back=6, max_results_per_query=100)
